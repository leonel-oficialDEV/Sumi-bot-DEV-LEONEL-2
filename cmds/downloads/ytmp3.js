...
async function getAudioFromApis(url) {
  const apis = [
    { 
      ...
      extractor: res => res?.status && res?.descarga?.url ? res.descarga.url : null 
    },
    { 
      api: 'Delirius', 
      endpoint: `https://api.delirius.store/download/ytmp3?url=${encodeURIComponent(url)}`, 
      extractor: res => res.status && res.data ? res.data.download : null 
    },
    { 
      api: 'Axi', 
      endpoint: `${global.APIs?.axi?.url || ''}/down/ytaudio?url=${encodeURIComponent(url)}`, 
      extractor: res => res?.resultado?.url_dl 
    },    
    { 
      api: 'Stellar', 
      endpoint: `${global.APIs?.stellar?.url || ''}/dl/ytdl?url=${encodeURIComponent(url)}&format=mp3&key=${global.APIs?.stellar?.key || ''}`, 
  ...
  for (const { api, endpoint, extractor } of apis) {
    try {
      if (!endpoint || endpoint.startsWith('undefined')) continue
...
async function getVideoFromApis(url) {
  const apis = [
    ...
    { 
      api: 'Stellar', 
      endpoint: `${global.APIs?.stellar?.url || ''}/dl/ytdl?url=${encodeURIComponent(url)}&format=mp4&key=${global.APIs?.stellar?.key || ''}`, 
      extractor: res => res.result?.download 
    }
  ]
